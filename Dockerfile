...
RUN curl -sSL https://storage.googleapis.com/golang/go${GO_VERSION}.src.tar.gz | tar -v -C /usr/local -xz \
	&& mkdir -p /go/bin
...
COPY vendor/src/code.google.com /usr/local/go/src/code.google.com
RUN set -x \
	&& export GOPATH="$(mktemp -d)" \
